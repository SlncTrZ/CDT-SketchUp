...
The measured native acceptance is current through contract `0.28` and its **67 MCP tools**. Source contract `0.29` adds caller-stable `execute_geometry.operation_id`; that additive recovery identity is offline/public-provider verified but is not yet promoted into the measured native baseline. In addition to the established semantic-loop baseline, SketchUp 2024 `24.0.594` / Ruby `3.2.2` has measured evidence for bounded three-level `target_context` edits/restoration, SHA-256 + `native_version` asset binding/reuse/drift rejection, generic bounded indexed mesh creation, exact manifold-solid clearance/overlap classification, content-addressed artifact sealing/verification, save/reopen evidence, and multi-step recovery/reconciliation. Native mesh cases include tetrahedron, frustum, four-section loft, ellipsoid, rounded prism and open curved molding/ribbon geometry; negative cases cover malformed topology, budget overflow before mutation and verified rollback.
