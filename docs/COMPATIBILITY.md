...
The current native acceptance includes the baseline live bridge and strict semantic-loop proof for box, face, isolated extrusion-to-group, absolute transform, manifold boolean, Group/ComponentInstance delete, strict group composition, strict component/instance semantics, strict copy/array/mirror duplication, strict tag/material assignment, strict curve/polyline primitives, strict profile sweep, read-only measurement/topology queries, allowlisted asset placement, real-world texture scale, camera/scene control, rooted document lifecycle and CAD integrity with safe repair. Contract `0.21` was live-smoked through the Streamable HTTP MCP surface with 64 discovered tools. Current contract `0.22` keeps the same 64-tool surface but corrects safety semantics for document I/O, canonical rooted-path containment, sanitized native-error responses, and unsaved-model protection before `model_open`; these corrections are automated-regression verified but have not yet been separately native-smoked. The registry continues to report `24.0.594` as measured runtime evidence while separately returning the currently observed SketchUp/Ruby runtime.
