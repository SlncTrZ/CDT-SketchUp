...
The current native acceptance includes the baseline live bridge and strict semantic-loop proof for box, face, isolated extrusion-to-group, absolute transform, manifold boolean, Group/ComponentInstance delete, strict group composition, strict component/instance semantics, strict copy/array/mirror duplication, strict tag/material assignment, strict curve/polyline primitives, strict profile sweep, read-only measurement/topology queries, allowlisted asset placement, real-world texture scale, camera/scene control, rooted document lifecycle and CAD integrity with safe repair. Contract `0.21` was live-smoked through the Streamable HTTP MCP surface with 64 discovered tools. Contract `0.22` corrected safety semantics for document I/O and rooted paths. Current contract `0.23` keeps the same 64 MCP tools and adds bounded topology closure/fingerprinting plus topology-aware raw Edge/Face deletion as an `execute_geometry` action. The 0.22/0.23 changes are automated-regression verified but have not yet been separately native-smoked; the registry continues to report `24.0.594` only as measured baseline evidence.
