...
The current implementation exposes a Python MCP provider plus a SketchUp Ruby bridge at contract `0.23`. The strict Semantic State Loop covers generic rollback-capable model mutations; rooted document save/open/export is explicitly modeled as a non-transactional external side effect with verified completion. Contract 0.23 adds a bounded raw-topology closure/fingerprint primitive and a topology-aware `delete_topology_entity` action under `execute_geometry`; every affected PID must stay inside the pre-mutation closure before commit. Model/asset/texture roots use canonical containment, `model_open` refuses unsaved active models, and earlier mutation tools remain compatibility paths where documented. The measured native SketchUp 2024 baseline predates the 0.22/0.23 changes, so compatibility documentation keeps runtime evidence separate from current contract semantics.
