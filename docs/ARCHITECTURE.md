...
The current implementation exposes a Python MCP provider plus a SketchUp Ruby bridge at contract `0.25`. The strict Semantic State Loop covers generic rollback-capable model mutations; rooted document save/open/export is explicitly modeled as a non-transactional external side effect with verified completion. Contracts 0.23–0.24 add bounded raw-topology closure/fingerprinting, topology-aware `delete_topology_entity`, and guarded connected-face `push_pull_topology_face` under `execute_geometry`. Contract 0.25 strengthens duplication semantics: direct copy and array paths propagate generic instance material, tag, name, hidden state and shadow flags, then verify those properties before commit. Raw delete constrains all touched old PIDs to the pre-closure; connected push/pull additionally constrains newly created PIDs to the bounded post-closure of the surviving source Face. The topology path and copy-fidelity path are natively measured on SketchUp 2024 `24.0.594` / Ruby `3.2.2`. Model/asset/texture roots use canonical containment and `model_open` refuses unsaved active models; those contract-0.22 document/path changes remain a separate native-acceptance scope.
