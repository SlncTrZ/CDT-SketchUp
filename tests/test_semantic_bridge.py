...
class SemanticBridgeTests(unittest.TestCase):
    ...
    def test_execute_geometry_supports_bounded_target_context_and_restores_caller_context(self) -> None:
        source = read_extension_sources()
        server_source = (ROOT / "src" / "cdt_sketchup" / "server.py").read_text(encoding="utf-8")
        self.assertIn("MAX_CONTEXT_DEPTH = 32", source)
        self.assertIn("TARGET_CONTEXT_KEYS = %w[instance_path].freeze", source)
        self.assertIn("def resolve_target_edit_context", source)
        self.assertIn("Sketchup::InstancePath.new(instances)", source)
        self.assertIn("model.active_path = target_path", source)
        self.assertIn("restore_active_path(model, original_path)", source)
        self.assertIn('"context_targeting"', source)
        self.assertIn('"edit_semantics" => "shared_definition"', source)
        self.assertIn('"instance_specific_edit" => "make_unique_first"', source)
        self.assertIn('"context_restore_failed"', source)
    ...
    def test_mesh_action_is_bounded_indexed_and_strict(self) -> None:
        source = read_extension_sources()
        self.assertIn('"create_mesh" => :execute_create_mesh', source)
        self.assertIn("MESH_PARAM_KEYS = %w[name points faces].freeze", source)
        self.assertIn("MAX_MESH_VERTICES = 2048", source)
        self.assertIn("MAX_MESH_FACES = 4096", source)
        self.assertIn("MAX_MESH_FACE_VERTICES = 16", source)
        self.assertIn("def validate_mesh_params", source)
        self.assertIn("def execute_create_mesh", source)
        self.assertIn('preflight_create_mesh(model, action_params) if action == "create_mesh"', source)
        self.assertIn('"mesh points must contain 3..2048 vertices"', source)
        self.assertIn('"mesh faces must contain 1..4096 polygons"', source)
        self.assertIn('"mesh face indexes must be unique within a face"', source)
        self.assertIn('"create_mesh requires expect.face_count"', source)
        self.assertIn('"create_mesh requires expect.vertex_count"', source)
        self.assertIn('when "create_polyline", "create_circle", "create_arc", "create_rectangle", "create_polygon", "create_mesh"', source)
    ...
    def test_measurement_queries_are_read_only_and_bounded(self) -> None:
        ...
        self.assertIn("def exact_spatial_relation", source)
        self.assertIn("entity.mesh(0)", source)
        self.assertIn("MAX_SPATIAL_TRIANGLES = 1024", source)
        self.assertIn("MAX_SPATIAL_PAIR_TESTS = 1048576", source)
        self.assertIn('"relationship" => relationship', source)
        self.assertIn('"surface_clearance"', source)
        self.assertIn('"bounds_overlap"', source)
        self.assertIn('"Exact spatial queries require manifold solids"', source)
    ...
    def test_asset_registry_binds_exact_bytes_version_and_loaded_definition_identity(self) -> None:
        source = read_extension_sources()
        self.assertIn('"available"', source)
        self.assertIn('"sha256"', source)
        self.assertIn('"native_version"', source)
        self.assertIn("Digest::SHA256.file", source)
        self.assertIn('"asset_identity_unverified"', source)
        self.assertIn('"asset_changed_during_load"', source)
        self.assertIn('"asset_definition_identity_mismatch"', source)
        self.assertIn("before_definition_guids", source)
        self.assertIn("ASSET_ATTRIBUTE_DICTIONARY", source)
        self.assertIn("model.set_attribute", source)
        self.assertIn("model.get_attribute", source)
        self.assertIn('"geometry_fingerprint"', source)
        self.assertIn("semantic_definition_geometry_fingerprint(definition)", source)
        self.assertIn('"asset_identity"', source)
        self.assertIn('"definition_reused"', source)

    def test_asset_rebinds_only_unused_unverified_definitions_after_verified_load(self) -> None:
        source = read_extension_sources()
        self.assertIn("def asset_definition_used?", source)
        self.assertIn("definition.count_used_instances", source)
        self.assertIn("expected = expected_definition_asset_identity(binding)", source)
        self.assertIn("semantic_asset_identity(model, definition) == expected", source)
        self.assertIn("return false unless definition.instances.empty?", source)
        self.assertIn("model.definitions.load(asset_path)", source)
        self.assertIn("post_load_sha256 = asset_file_sha256(asset_path)", source)
        self.assertIn("bind_asset_definition_identity(model, definition, binding)", source)
        self.assertIn('"A loaded definition for this asset is already in use but does not match the verified registry identity"', source)
    ...
    def test_artifact_seal_is_content_addressed_bounded_and_stale_aware(self) -> None:
        source = read_extension_sources()
        self.assertIn('"artifact_seal" => :handle_artifact_seal', source)
        self.assertIn('"artifact_verify" => :handle_artifact_verify', source)
        self.assertIn("MAX_ARTIFACT_BYTES = 1073741824", source)
        self.assertIn("def artifact_sha256", source)
        self.assertIn("Digest::SHA256.file(path).hexdigest", source)
        self.assertIn('"artifact_changed_during_seal"', source)
        self.assertIn('"artifact_seal_corrupt"', source)
        self.assertIn('"artifact_stale"', source)
        self.assertIn("same_canonical_file?", source)
        self.assertIn("canonical_contained_path(", source)
        self.assertIn("model.modified?", source)
